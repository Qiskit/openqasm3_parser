...
impl<'t> Parser<'t> {
    ...
    // // GJL for debugging only
    // pub(crate) fn getsteps(&self) -> usize {
    ...
    // Maybe don't want to delete this cruft ?
    // /// Advances the parser by one token
    ...
    /// Create an error node and consume the next token unless current token is { or }
    ///
    /// If the next token is { or }, then record the error message and return without adding to the tree.
    /// Otherwise, record the error message and consume a token and place it under a new ERROR node.
    pub(crate) fn err_and_bump(&mut self, message: &str) {
        self.err_recover(message, TokenSet::EMPTY);
    }

    /// The r-a doc-comment here is copied from above and is incorrect. So have to write our own.
    /// If current token is in the `recovery` set or is { or }, then record the error message and return.
    /// Otherwise create an ERROR node. An ERROR node is a node entered in the parse tree for something we don't
    /// understand. One token is consumed and is placed under the node. The error message `message` is associated
    /// with the node.
