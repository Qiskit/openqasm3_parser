...
//use semantic_ast::syntax_to_semantics as synsem;
...
fn _parse_file(file_path: String) -> (ast::Program, symbols::SymbolTable) {
    // let source_text = fs::read_to_string(file_path.clone())
    //     .expect(format!("Unable to read file {:?}", file_path).as_str());
    let pbuf = PathBuf::from(file_path);
    let result = semantic_ast::syntax_to_semantics::parse_source_file(&pbuf);
    result.print_errors();
