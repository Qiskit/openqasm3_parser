...
use semantic_ast::{ast, types, symbols};
...
#[pyclass]
#[derive(Clone)]
pub struct ClassicalDeclaration(ast::ClassicalDeclaration);
...
#[pyclass]
#[derive(Clone)]
pub struct QuantumDeclaration(ast::QuantumDeclaration);
...
#[pyclass]
#[derive(Clone)]
pub struct GateCall(ast::GateCall);
...
#[derive(Clone)]
pub struct Stmt(ast::Stmt);

impl IntoPy<PyObject> for Stmt {
    fn into_py(self, py: Python<'_>) -> PyObject {
        match self.0 {
            ast::Stmt::QuantumDeclaration(qdecl) => QuantumDeclaration(qdecl).into_py(py),
            ast::Stmt::ClassicalDeclaration(cdecl) => ClassicalDeclaration(cdecl).into_py(py),
            ast::Stmt::GateCall(gate_call) => GateCall(gate_call).into_py(py),
            _ => py.None()
        }
    }
}
...
#[pyclass]
#[derive(Clone)]
pub struct Program(pub ast::Program);
...
#[pyclass]
#[derive(Clone)]
pub struct BoolLiteral(ast::BoolLiteral);
...
// #[pyclass]
#[derive(Clone)]
pub struct Expr<'a>(&'a ast::Expr);

impl IntoPy<PyObject> for Expr<'_> {
    fn into_py(self, py: Python<'_>) -> PyObject {
        match self.0 {
            ast::Expr::Literal(ast::Literal::Bool(bool_literal)) => {
                BoolLiteral(bool_literal.clone()).into_py(py)
            },
            ast::Expr::GateOperand(ast::GateOperand::Identifier(ident)) => {
//                dbg!(ident.clone());
...
pub fn get_a_qdecl() -> Stmt {
    use semantic_ast::symbols::{SymbolTable};
    use semantic_ast::types::{Type, IsConst};
    let mut table = SymbolTable::new();
    let x = table.new_binding("q", &Type::Bool(IsConst::False));
    let qd = ast::QuantumDeclaration::new(x);
    Stmt(ast::Stmt::QuantumDeclaration(qd))
}
...
pub fn cdeclaration(py: Python<'_>) -> (PyObject, SymbolTable) {
    ...
    let initializer = Some(ast::BoolLiteral::new(false).to_texpr());
    let decl = ast::ClassicalDeclaration::new(x, initializer).to_stmt();
