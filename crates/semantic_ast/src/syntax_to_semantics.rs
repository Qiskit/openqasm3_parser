// Build the AST defined in ast.rs from the syntactic AST
...
use crate::ast;
use crate::types;
use crate::types::{Type, IsConst, ArrayDims};

use crate::symbols::{ScopeType, SymbolErrorTrait, SymbolIdResult};
use oq3_syntax::ast as synast; // Syntactic AST
use source_file::{SourceTrait, SourceFile, SourceString};
use crate::semantic_error::{SemanticErrorList, SemanticErrorKind::*};
...
// traits
use synast::{HasModuleItem, HasName, HasTextName, HasArgList};

pub struct ParseResult<T: SourceTrait> {
    syntax_result: T, // syntax tree and errors
    context: Context, // semantic ast and errors
...
impl ParseResult<SourceString> {
    pub fn print_errors(&self) {
        ...
        self.context.errors().print_errors_no_file(synres.fake_file_path(), synres.source());
    }
}

/// Parse string containing source to semantic AST.
/// Fake file name is used for printing diagnostics.
pub fn parse_source_string<T: ToString>(source: T, fake_file_path: Option<&str>) -> ParseResult<SourceString> {
    ...
    parse_source(parsed_source, &file_path)
}

/// Parse source file to semantic AST
...
fn parse_source<T: SourceTrait>(parsed_source: T, file_path: &Path) -> ParseResult<T> {
    let context = Context::new(file_path.to_path_buf());
    if parsed_source.any_parse_errors() { // on syntax errors, do not continue with semantic analysis.
        return ParseResult { syntax_result: parsed_source, context: context.clone(), have_syntax_errors: true };
    }
    ...
    ParseResult { syntax_result: parsed_source, context: context.clone(), have_syntax_errors: false }
}

// FIXME: it is probably possible to encapsulate the manipulations of (context, errors).
pub fn syntax_to_semantic<T: SourceTrait>(parsed_source: &T, mut context: Context, errors: SemanticErrorList) -> (Context, SemanticErrorList) {
    ...
    for parse_item in parse_tree.statements() {
        let stmt = match parse_item {
            ...
            // Include does not go in the AST, instead it is evaluated.
            // So we include the parsed code, collect errors, and return `None`.
            synast::Stmt::Item(synast::Item::Include(_)) => {
                // FIXME: Need to check that we are in global scope.
                ...
                (context, errors_in_included) = syntax_to_semantic(included_parsed_source, context, errors_in_included);
            ...
            synast::Stmt::Item(item) => {
                from_item(item, &mut context)
            },

            synast::Stmt::ExprStmt(expr_stmt) => {
                from_expr_stmt(expr_stmt, &mut context)
            },
        };
        if let Some(stmt) = stmt { context.program.insert_stmt(stmt) }
    }
...
fn from_expr_stmt(expr_stmt: synast::ExprStmt, context: &mut Context) -> Option<ast::Stmt> {
    let expr = from_expr(expr_stmt.expr().unwrap(), context);
    expr.map_or_else(|| panic!("expr::ExprStmt is None"), |ex| Some(ast::Stmt::ExprStmt(ex)))
}

fn from_expr(expr: synast::Expr, context: &mut Context) -> Option<ast::TExpr> {
    match expr {
        synast::Expr::ParenExpr(paren_expr) => {
        ...
        synast::Expr::BinExpr(bin_expr) => {
            ...
            Some(ast::BinaryExpr::new_texpr_with_cast(op, left, right))
        },

        synast::Expr::Literal(ref literal) => {
        ...
        synast::Expr::HardwareQubit(hwq) => {
            Some(ast_hardware_qubit(&hwq).to_texpr())
        },
        ...
        synast::Expr::RangeExpr(range_expr) => {
            ...
            let range = ast::Range::new(start, step, stop);
        ...
        synast::Expr::IndexExpr(index_expr) => {
            ...
            Some(ast::IndexExpression::new(expr.unwrap(), index).to_texpr())
        }
        ...
        synast::Expr::MeasureExpression(_measure_expr) => {
            None
        },
        _ => {
...
fn from_index_operator(index_op: synast::IndexOperator, context: &mut Context) -> ast::IndexOperator {
    match index_op.index_kind().unwrap() {
        synast::IndexKind::SetExpression(set_expression) =>
            ast::IndexOperator::SetExpression(from_set_expression(set_expression, context)),

        synast::IndexKind::ExpressionList(expression_list) =>
            ast::IndexOperator::ExpressionList(from_expression_list(expression_list, context)),
    }
}

fn from_set_expression(set_expression: synast::SetExpression, context: &mut Context) -> ast::SetExpression {
    ast::SetExpression::new(inner_expression_list(set_expression.expression_list().unwrap(), context))
}

fn from_expression_list(expression_list: synast::ExpressionList, context: &mut Context) -> ast::ExpressionList {
    ast::ExpressionList::new(inner_expression_list(expression_list, context))
}

#[allow(clippy::filter_map_identity)]
fn inner_expression_list(expression_list: synast::ExpressionList, context: &mut Context) -> Vec<ast::TExpr> {
...
fn from_binary_op(synast_op: synast::BinaryOp) -> ast::BinaryOp {
    match synast_op {
        synast::BinaryOp::ArithOp(arith_op) => {
            use synast::ArithOp::*;
            use ast::BinaryOp::ArithOp;
            match arith_op {
                Add => ArithOp(ast::ArithOp::Add),
                Mul => ArithOp(ast::ArithOp::Mul),
                Sub => ArithOp(ast::ArithOp::Sub),
                Div => ArithOp(ast::ArithOp::Div),
        ...
        synast::BinaryOp::CmpOp(cmp_op) => {
            use synast::CmpOp::*;
            use ast::BinaryOp::CmpOp;
            match cmp_op {
                Eq {negated: false} => CmpOp(ast::CmpOp::Eq),
                _ => todo!(),
            }
        },
        _ => {
...
fn from_item(item: synast::Item, context: &mut Context) -> Option<ast::Stmt> {
    match item {
        synast::Item::IfStmt(if_stmt) => {
            ...
            Some(ast::If::new(condition.unwrap(), then_branch, else_branch).to_stmt())
        },
        ...
        synast::Item::WhileStmt(while_stmt) => {
            ...
            Some(ast::While::new(condition.unwrap(), loop_body).to_stmt())
        },
        ...
        synast::Item::QuantumDeclarationStatement(q_decl) => {
            ...
            let q_decl_ast = ast::QuantumDeclaration::new(symbol_id);
            Some(ast::Stmt::QuantumDeclaration(q_decl_ast))
        }
        ...
        synast::Item::BreakStmt(_) => {
            Some(ast::Stmt::Break)
        }

        synast::Item::ContinueStmt(_) => {
            Some(ast::Stmt::Continue)
        }

        synast::Item::EndStmt(_) => {
            Some(ast::Stmt::End)
        }
        ...
        synast::Item::Gate(gate) => {
            ...
            let gate_name_symbol_id = context.new_binding(name_node.string().as_ref(), &Type::Gate, &name_node);
            ...
            Some(ast::Gate::new(gate_name_symbol_id, params, qubits, block).to_stmt())
        }
        ...
        synast::Item::GateCallStmt(gate_call) => {
            ...
            let gate_operands = gate_call.qubit_list().unwrap().gate_operands()
                .map(|qubit|
                     match qubit {
                         synast::GateOperand::HardwareQubit(ref hwq) => {
                             ast::GateOperand::HardwareQubit(ast_hardware_qubit(hwq)).to_texpr(Type::HardwareQubit)
                         }
                         synast::GateOperand::Identifier(identifier) => {
                             let (astidentifier, typ) = ast_identifier(&identifier, context);
                             ast::GateOperand::Identifier(astidentifier).to_texpr(typ)
                         }
                         synast::GateOperand::IndexedIdentifier(indexed_identifier) => {
                             ...
                             ast::GateOperand::IndexedIdentifier(indexed_identifier).to_texpr(typ)
                         }
                     }
                )
                .collect();

            let param_list = gate_call.arg_list()
            ...
            let (symbol_result, _typ) = context.lookup_symbol(gate_name.as_ref(), &gate_id.unwrap()).as_tuple();
            Some(ast::Stmt::GateCall(ast::GateCall::new(symbol_result, param_list, gate_operands, None)))
        },

        synast::Item::Include(_include) => {
            unreachable!()
        },
...
fn from_block_expr(block_synast: synast::BlockExpr, context: &mut Context) -> ast::Block {
    let mut block = ast::Block::new();

    for parse_item in block_synast.statements() {
        let stmt = match parse_item {
            synast::Stmt::Item(item) => {
                from_item(item, context)
            },
            synast::Stmt::ExprStmt(expr_stmt) => {
                from_expr_stmt(expr_stmt, context)
            },
        };
        if let Some(stmt) = stmt { block.insert_stmt(stmt) }
    }
    block
}

fn from_classical_declaration_statement(type_decl: &synast::ClassicalDeclarationStatement, context: &mut Context) -> ast::Stmt {
    ...
    let width = match scalar_type.designator().and_then(|desg| desg.expr()) {
        Some(synast::Expr::Literal(ref literal)) => {
            match literal.kind() {
                ...
                _ => { // FIXME: This error should be done when validating syntax. Before the semantic analysis
    ...
    let initializer = type_decl.expr().and_then(|initializer| from_expr(initializer, context));
    ...
    ast::ClassicalDeclaration::new(symbol_id, initializer).to_stmt()
}

// FIXME: In oq3_syntax we have both Name and Identifier. We only need one, I think.
// This will be changed.

fn from_assignment_stmt(assignment_stmt: &synast::AssignmentStmt, context: &mut Context) -> Option<ast::Stmt> {
    ...
    let lvalue = ast::LValue::Identifier(symbol_id);
    let ret_stmt = Some(ast::Assignment::new(lvalue, expr.unwrap()).to_stmt());
...
fn ast_hardware_qubit(hwq: &synast::HardwareQubit) -> ast::HardwareQubit {
    ast::HardwareQubit::new(hwq.string())
}

fn ast_identifier(identifier: &synast::Identifier, context: &mut Context) -> (ast::Identifier, Type) {
    let name_str = identifier.string();
    let (symbol_id, typ) = context.lookup_symbol(name_str.as_str(), identifier).as_tuple();
    (ast::Identifier::new(name_str, symbol_id), typ)
}

fn ast_indexed_identifier(indexed_identifier: &synast::IndexedIdentifier, context: &mut Context) -> (ast::IndexedIdentifier, Type) {
    let identifier = indexed_identifier.identifier().unwrap().string();
    let (symbol_id, typ) = context.lookup_symbol(identifier.as_ref(), indexed_identifier).as_tuple();
    let indexes = indexed_identifier.index_operators()
        .map(|index| from_index_operator(index, context))
        .collect();
    (ast::IndexedIdentifier::new(symbol_id, indexes), typ)
}

// Bind all parameter names to new symbols. Assume they all have common type `typ`.
// Log RedeclarationError when it occurs.
fn bind_parameter_list(inparam_list: Option<synast::ParamList> , typ: &Type, context: &mut Context) -> Option<Vec<SymbolIdResult>> {
    inparam_list
        .map(|param_list|
             param_list.params()
