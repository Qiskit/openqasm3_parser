...
impl Symbol {
    fn new<T: ToString>(name: T, typ: Type) -> Symbol {
        Symbol { name: name.to_string(), typ }
    }
...
impl SymbolRecord<'_> {
    pub fn new(symbol: &Symbol, symbol_id: SymbolId, scope_level: usize) -> SymbolRecord<'_> {
        SymbolRecord { symbol, symbol_id, scope_level }
    }
...
impl SymbolTable {
    ...
    pub fn new_binding(&mut self, name: &str, typ: &Type) -> Result<SymbolId, SymbolError> {
        ...
        let symbol = Symbol::new(name, typ.clone());
        ...
        self.current_scope_mut().insert(name, current_symbol_id.clone());
    ...
    fn current_scope_type(&self) -> ScopeType {
