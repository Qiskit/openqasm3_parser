// The definition of the typed (semantic) AST as well as the API for using it.
// Construction of this typed AST from syntactic AST is in string_to_semantic.rs.
...
// Because, although this AST can assume synactic correctness, we need to track
// semantic errors and continue to build the semantic AST.
...
impl Program {
    pub fn new() -> Program {
        Program { version: None, stmts: Vec::<Stmt>::new() }
    }
    ...
    /// Print the AST using the pretty print `Debug` trait.
    pub fn print_ast_debug_pretty(&self) {
        for stmt in self.iter() {
            println!("{:#?}", stmt);
        }
    }

    /// Print the AST using the `Debug` trait.
    pub fn print_ast_debug(&self) {
        for stmt in self.iter() {
            println!("{:?}", stmt);
        }
    }
...
// FIXME: Rename `Gate` to `DeclareGate` or `GateDef` or s.t. like that
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub enum Stmt {
    Alias, // stub
...
impl Include {
    pub fn new<T: ToString>(file_path: T) -> Include {
        Include { file_path: file_path.to_string() }
    }
...
impl Annotation {
    pub fn new<T: ToString>(kind: T, body: Option<String>) -> Annotation {
        Annotation { kind: kind.to_string(), body }
    }
...
impl AnnotatedStmt {
    ...
    pub fn new(stmt: Stmt, annotations: Vec<Annotation>) -> AnnotatedStmt {
        ...
        AnnotatedStmt { stmt: Box::new(stmt), annotations }
    }
...
impl IndexExpression {
    pub fn new(expr: TExpr, index: IndexOperator) -> IndexExpression {
        IndexExpression { expr: Box::new(expr), index }
    }

    pub fn to_texpr(self) -> TExpr {
        TExpr::new(Expr::IndexExpression(self), Type::Void)
    }
}
...
impl IndexedIdentifier {
    ...
    pub fn to_texpr(self) -> TExpr {
        TExpr::new(Expr::IndexedIdentifier(self), Type::Void)
    }
}
...
impl HardwareQubit {
    pub fn new<T: ToString> (identifier: T) -> HardwareQubit {
        HardwareQubit { identifier: identifier.to_string() }
    }
...
// `symbol` is `None` when re-declaration encountered.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
...
impl ClassicalDeclaration {
    pub fn new(name: SymbolIdResult, initializer: Option<TExpr>) -> ClassicalDeclaration {
        ClassicalDeclaration { name, initializer: initializer.map(Box::new) }
    }
    ...
    pub fn to_stmt(self) -> Stmt {
        Stmt::ClassicalDeclaration(self)
    }
}
...
impl QuantumDeclaration {
    pub fn new(name: SymbolIdResult) -> QuantumDeclaration {
        QuantumDeclaration { name }
    }
...
impl Gate {
    pub fn new(name: SymbolIdResult, params: Option<Vec<SymbolIdResult>>, qubits: Vec<SymbolIdResult>, block: Block) -> Gate {
        Gate {name, params, qubits, block }
    }
...
impl GateCall {
    pub fn new(name: SymbolIdResult, params: Option<Vec<TExpr>>, qubits: Vec<TExpr>, modifier: Option<GateModifier>) -> GateCall {
        GateCall {name, params, qubits, modifier}
    }
...
// Typed expression. An expression decorated with OQ3 type.
...
// However when consuming the semantic AST at later stage, dynamic dispatch may be required.
...
impl Cast {
    pub fn new(operand: TExpr, typ: Type) -> Cast {
        Cast { operand : Box::new(operand), typ }
    }
...
impl BoolLiteral {
    pub fn new<T>(value: T) -> BoolLiteral where T: Into<bool> {
...
impl IntLiteral {
    pub fn new<T>(value: T) -> IntLiteral where T: Into<u128> {
...
impl BitStringLiteral {
    pub fn new<T: ToString>(value: T) -> BitStringLiteral {
        BitStringLiteral { value: value.to_string() }
    }
    ...
    pub fn to_texpr(self) -> TExpr {
        ...
        TExpr::new(self.to_expr(), Type::BitArray(ArrayDims::D1(width), IsConst::True))
    }
}
...
impl UnaryExpr {
    pub fn new(op: UnaryOp, operand: TExpr) -> UnaryExpr {
        UnaryExpr { op, operand: Box::new(operand) }
    }
...
impl BinaryExpr {
    ...
    pub fn new_texpr_with_cast(op: BinaryOp, left: TExpr, right: TExpr) -> TExpr {
        ...
        let new_left =
            if &promoted_type == left_type {
                left
            } else {
                Cast::new(left, promoted_type.clone()).to_texpr()
            };
        let new_right =
            if &promoted_type == right_type {
                right
            } else {
...
impl Identifier {
    pub fn new<T: ToString>(name: T, symbol: SymbolIdResult) -> Identifier {
        Identifier { name: name.to_string(), symbol }
    }
...
impl Range {
    pub fn new(start: TExpr, step: Option<TExpr>, stop: TExpr) -> Range {
        Range { start: Box::new(start), step: Box::new(step), stop: Box::new(stop) }
    }
...
impl Pragma {
    pub fn new<T: ToString>(pragma: T) -> Pragma {
        Pragma { pragma: pragma.to_string() }
    }
