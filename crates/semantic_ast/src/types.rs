...
impl Type {
    ...
    pub fn is_scalar(&self) -> bool {
        use Type::*;
        matches!(self, Bit(..) | Int(..) | UInt(..) | Float(..) | Angle(..) | Complex(..) | Bool(..) | Duration(..) | Stretch(..))
    }
...
pub fn promote_types(ty1: &Type, ty2: &Type) -> Type {
    ...
    match (ty1, ty2) {
        ...
        (UInt(..), UInt(..)) => {
            UInt(promote_width(ty1, ty2), isconst)
        }
        (Int(..), Float(..)) => ty2.clone(),
        (Float(..), Int(..)) => ty1.clone(),
        _ => Void,
...
pub(crate) fn can_cast_loose(from_type: &Type, to_type: &Type) -> bool {
    use Type::*;
    equal_up_to_constness(from_type, to_type) ||
        !matches!((from_type, to_type), (Bit(..), Bit(..)))
}
